...
from ..utils.config import USER_ID, ITEM_ID, EQUITY_DATE, EQUITY_PRICE


def preprocess_trans_buy():
...
def preprocess_crm(cal_var: bool = False, predict_year: int = 202012,
                   normalize: bool = True, group_age: bool = True):
    """_summary_

    Args:
        cal_var (bool, optional): 計算 crm 欄位變動值. Defaults to False.
        predict_year (int, optional): _description_. Defaults to 202012.
        normalize (bool, optional): _description_. Defaults to True.
        group_age (bool, optional): _description_. Defaults to True.
    """
    ...
    crm.columns = [
        ...
        'age', 'monthly_trade_vol', 'stock_inventory_val', 'KPI'
    ]
    ...
    if cal_var:
        ...
        rename_dict = {diff_cols[i]: v +
                       "_diff" for i, v in enumerate(diff_cols)}
        ...
        for u in tqdm(crm.id_number.unique().tolist()):
            ...
            # 註：改用 shift 應該可以加速
            tmp = user_df[diff_cols].diff().replace(
                np.nan, 0).rename(rename_dict, axis=1)
    ...
    print("crm preprocessing completed.")


def preprocess_equity():
    # load path
    ...
    diff_df = pd.DataFrame([], columns=[
        'diff_month', 'diff_quarter', 'diff_half_year',
        'diff_year', 'diff_2year'
    ])

    for i in tqdm(equity.fund_id.unique().tolist()):
        item_df = equity[equity.fund_id == i]
        tmp = item_df[[EQUITY_PRICE]].diff().replace(
            np.nan, 0).rename({EQUITY_PRICE: 'diff_month'}, axis=1)
        tmp['diff_quarter'] = item_df[[EQUITY_PRICE]].diff(periods=3).replace(
            np.nan, 0).rename({EQUITY_PRICE: 'value_diff'}, axis=1)
        tmp['diff_half_year'] = item_df[[EQUITY_PRICE]].diff(periods=6)\
            .replace(np.nan, 0).rename({EQUITY_PRICE: 'value_diff'}, axis=1)
        tmp['diff_year'] = item_df[[EQUITY_PRICE]].diff(periods=12).replace(
            np.nan, 0).rename({EQUITY_PRICE: 'value_diff'}, axis=1)
        tmp['diff_2year'] = item_df[[EQUITY_PRICE]].diff(periods=24).replace(
            np.nan, 0).rename({EQUITY_PRICE: 'value_diff'}, axis=1)
        diff_df = diff_df.append(tmp)
...
def preprocess_fund_web():
    ...
    print("fund_web preprocessing completed.")
...
def preprocess_fund():
    ...
    with open(idx_save_path, 'w') as f:
        json.dump(i2idx, f)
    print("fund preprocessing completed.")


def preprocess_crm_cluster(savename="crm_for_clustering_2020.csv"):
    crm_new_path = DATA_PATH / "origin/crm_new.csv"
    fund_crm_feature_path = DATA_PATH / "origin/fund_crm_feature.csv"

    crm_new = pd.read_csv(crm_new_path, encoding="big5")
    crm_non_trade = pd.read_csv(fund_crm_feature_path, encoding="big5")

    # 確保 yyyymm 欄位大小寫一致
    crm_non_trade.columns = crm_non_trade.columns.str.replace(
        'YYYYMM', 'yyyymm')
    ...
    crm_non_trade.columns = crm_non_trade.columns.str.replace(
        '身分證字號', 'id_number')  # 把身分證字號轉 id_number，確保與其他資料表一致
    ...
    crm_for_clustering = pd.get_dummies(crm_for_clustering,
                                        columns=multi_category_col)
    crm_for_clustering = crm_for_clustering.replace(-np.inf, -1)

    crm_for_clustering.to_csv(savename, index=False)

    print("crm_for_clustering preprocessing completed.")
